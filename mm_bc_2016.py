...
loc = 'C:\\Users\\brett\\Documents\\mm_2016_ml\\datasets'
...
train, test, train_target, test_target = train_test_split(t_games[test_atts], t_games.W_L, test_size = 0.2, random_state = 11)
...
# knn
knn5 = neighbors.KNeighborsClassifier(25)
knn9 = neighbors.KNeighborsClassifier(9)
knn15 = neighbors.KNeighborsClassifier(15)
knn5d = neighbors.KNeighborsClassifier(25, weights = 'distance')
knn9d = neighbors.KNeighborsClassifier(9, weights = 'distance')
knn15d = neighbors.KNeighborsClassifier(15, weights = 'distance')
...
classification_models = {'knn5':knn5, 
                         'knn9':knn9,
                         'knn15':knn15,
                         'knn5d':knn5d,
                         'knn9d':knn9d,
...
# Create brackets
slot_16 = t_slots[t_slots.Season == 2016]
seed_16 = t_seeds[['Seed', 'Team']][t_seeds.Season ==2016]
...
# select teams to evalutate
null_check = ss_16i.STeam[ss_16i.STeam == 0].count() + ss_16i.WTeam[ss_16i.WTeam == 0].count()
ss_16['evaluated'] = False
i = 0

# loop through all rounds and models to make predictions
while null_check > 0:  
       
    Rounds.append([i])
    Rounds[i] = {}   
    for model_name, model in tourney_models.iteritems():  
        
        if i > 0:
            ss_16i = Rounds[i - 1][model_name + '_ss_16i']
        else:
            ss_16i = ss_16
        
        # Collect games ready to evaluate
        ss_eval = ss_16i[(ss_16i.STeam > 0) \
                            & (ss_16i.WTeam > 0) \
                            & (ss_16i.evaluated == False)]
        
        # append team record info                
        ...
        ss_STeam = ss_16i.merge(winner, 
                               left_on = 'Strongseed', 
                               right_on = 'new_seed', 
                               how = 'left')
        
                       
        ss_WTeam = ss_16i.merge(winner, 
                               left_on = 'Weakseed', 
                               right_on = 'new_seed', 
                               how = 'left')
        ...
        ss_16i.STeam = ss_16i.STeam.add(ss_STeam.Team)        
        ss_16i.WTeam = ss_16i.WTeam.add(ss_WTeam.Team)

        # check off games evaludated
        ss_16i['evaluated'] = (ss_16i.STeam > 0) & (ss_16i.WTeam > 0)                                        
        
        Rounds[i][model_name + '_ss_16i'] = ss_16i
        
  
    # check nulls remaining and continue to iterate    
    null_check = ss_16i.STeam[ss_16i.STeam == 0].count() + ss_16i.WTeam[ss_16i.WTeam == 0].count()
