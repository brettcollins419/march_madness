...
def createMatchups(matchupDF, 
    ...
    colCount = pd.DataFrame(map(lambda c: c.replace(teamLabel1, '').replace(teamLabel2, '').replace('onentID', 'opponentID'), 
    ...
    matchupCols = filter(lambda c: matchupNew['team{}'.format(c)].dtype.hasobject == True,
    ...
    if calculateDelta == True:       
        for col in deltaCols:
            matchupNew.loc[:, '{}Delta'.format(col)] = (matchupNew.loc[:, 'team{}'.format(col)]
                                                        - matchupNew.loc[:, 'opp{}'.format(col)])
    if calculateMatchup == True:
        for col in list(set(matchupCols + extraMatchupCols)):
            matchupNew['{}Matchup'.format(col)] =  createMatchupField(matchupNew, 'team{}'.format(col), 'opp{}'.format(col), sort = False)
...
strengthMetrics = filter(lambda metric: metric.find('Strength') >= 0, matchups.columns.tolist())
...
strengthMetrics = filter(lambda metric: len(re.findall('.*Strength.*2', metric)) > 0, matchups.columns.tolist())

strengthDF2 = matchups.groupby(['Season', 'TeamID']).agg(dict(zip(strengthMetrics, repeat(np.mean, len(strengthMetrics)))))
# Scale Data between 0 and 1 using minmax to avoid negatives and append values as '[metric]Rank'
...
#x = matchups[(matchups['Season'] == 1990) & (matchups['TeamID'] == 1258)]
...
strengthMatchupsTourney = createMatchups(matchupDF = dataDict['tGamesC'][['Season', 'WTeamID', 'LTeamID']],
                                         statsDF = strengthDF.loc[:, filter(lambda metric: metric.endswith('Delta') | metric.endswith('Rank'),
                                                                            strengthDF.columns.tolist())],
...
fig, ax = plt.subplots(1)
sns.boxplot(x='variable', y='value', data = pd.melt(strengthMatchupsTourney.drop(['Season', 'WTeamID', 'LTeamID'], axis = 1)))
fig.tight_layout()
...
fig, ax = plt.subplots(1)
sns.boxplot(x='variable', y='value', data = pd.melt(strengthMatchupsTourney), ax = ax)
fig.tight_layout()
...
# Plot topN results
fig, ax = plt.subplots(1)
sns.scatterplot(x = map(lambda s: float(re.findall('[0-9]+', s)[0]), 
                        list(strengthMatchupsTourneyResultsTopN.index.get_values())),
                y = strengthMatchupsTourneyResultsTopN.loc[:, 'avg'], ax = ax)
fig.tight_layout()    
  


# Merge team strength and wins against top 50 teams metrics to team metrics
dataDict['{}TeamSeasonStats'.format('rGamesC')] = dataDict['{}TeamSeasonStats'.format('rGamesC')].merge(strengthDF.loc[:, ['spreadStrengthOppStrengthRank', 'wins50']], left_index = True, right_index = True)  
...
for df in filter(lambda g: g.startswith('t'), gamesData):
    ...
    regDF = df.replace('t', 'r')

    # Create matchups from base model dataset   
    dataDict[df + 'singleTeam'] = buildSingleTeam(df = dataDict[df])
    

    dataDict[df + 'modelData'] = createMatchups(matchupDF = dataDict[df + 'singleTeam'].loc[:, ['Season', 'TeamID', 'opponentID', 'win']], 
                                                statsDF = dataDict[regDF + 'TeamSeasonStats'],
                                                returnStatCols = True,
                                                calculateDelta = True,
...
x = dataDict['tGamesCmodelData']

# Win probability based on seed rank difference
srDeltaStats = x.groupby('seedRankDelta').agg({'TeamID': np.count_nonzero,
                                                       'win' : np.mean
                                                     }).rename(columns = {'TeamID': 'numGames', 'win': 'winPct'})
    


x2 = x.groupby(['teamseedRank', 'oppseedRank', 'seedRankDelta']).agg({'TeamID': np.count_nonzero,
                                                       'win' : np.mean
                                                     }).rename(columns = {'TeamID': 'numGames', 'win': 'winPct'})
...
fig, ax = plt.subplots(1)
sns.heatmap(x2.pivot(index = 'oppseedRank', columns = 'teamseedRank', values = 'winPct'), annot = True, fmt = "d")
 

fig, ax = plt.subplots(nrows = 2, ncols = 1, sharex = True)   
sns.scatterplot(x2['teamseedRank'], x2['oppseedRank'], size =x2['numGames'], hue = x2['winPct'], ax = ax[0])
sns.scatterplot(x2['teamseedRank'], x2['seedRankDelta'], size =x2['numGames'], hue = x2['winPct'], ax = ax[1])



fig, ax = plt.subplots(1)
sns.boxplot(x = 'seedRankDelta', y = 'winPct', data = x2, ax = ax)
sns.swarmplot(x = 'seedRankDelta', y = 'winPct', data = x2, color = 'k', ax = ax)
plt.grid()
...
for df in ('tGamesC', 
    ...
    modelCols = filter(lambda c: (dataDict[df + 'modelData'][c].dtype.hasobject == False)
    ...
    data = dataDict[df + 'modelData'][dataDict[df + 'modelData'].loc[:, 'Season'] >= 1985]
...
#    map(lambda d: d.update({'pca__n_components' : range(3, numIndCols, numIndCols // numPCASplits)}), paramGrid)    
    ...
    modelDict[df]['analysis'] = modelAnalysisPipeline(modelPipe = pipe,
                          data = dataDict[df + 'modelData'][dataDict[df + 'modelData']['Season'] >= 1985],
                          indCols = indCols2,
                          targetCol = 'win',
                          testTrainSplit = 0.2,
                          gridSearch=True,
